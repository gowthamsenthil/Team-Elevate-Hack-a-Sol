...
if player and team:
    ...
    if player_type == 'Batsman':
        ...
        with info1:
            ...
            st.metric("Boundary Percentage", f"{stats['Boundary Percentage']:.0f} %")
    ...
    with predictions:
        ...
        if player_type == 'Batsman':
            ...
            raw_data = raw_data.sort_values(by='date')  # Sort by date to ensure chronological order

            # Aggregate total runs scored by the batter per match (using match_id)
            agg_df = raw_data.groupby(['match_id', 'date', 'batter'])['batsman_runs'].sum().reset_index()

            # Add a sequential match number
            agg_df['match_number'] = range(1, len(agg_df) + 1)

            # Create the line plot
            fig = px.line(agg_df, x='match_number', y='batsman_runs', color='batter',
                        labels={'match_number': 'Match Number', 'batsman_runs': 'Total Runs'},
                        title=f'{player} Runs per Match Against {team}',
                        markers=True)
            fig.update_traces(line=dict(color='#FF4B4B'),
                  hovertemplate='<b>Match Number:</b> %{x}<br>' +
                                '<b>Total Runs:</b> %{y}<extra></extra>',
                  customdata=agg_df[['date']].values)
            fig.update_layout(title={'x': 0.20})
            # Display the plot (for Streamlit, use st.plotly_chart(fig))
            #fig.show()
           ...
           # Format the DataFrame to show 2 decimal places and add % symbol for Century
            ...
            # Define two tabs with labels
            tab1, tab2 = st.tabs(["Statistics", "Predictions"])

            with tab1:

                st.plotly_chart(fig)  # Display the Plotly chart in the first tab
                
            with tab2:

                st.table(op)  # Display the styled DataFrame table in the second tab
                st.markdown(f"""
        ...
        if player_type == 'Bowler': 
            ...
            raw_data = raw_data[(raw_data['bowler'] == player) & (raw_data['batting_team'] == team)]

            raw_data['date'] = pd.to_datetime(raw_data['date'])  
            raw_data = raw_data.sort_values(by='date')  # Sort by date to ensure chronological order

            # Aggregate total runs scored by the batter per match (using match_id)
            agg_df = raw_data.groupby(['match_id', 'date', 'bowler'])['is_wicket'].sum().reset_index()

            # Add a sequential match number
            agg_df['match_number'] = range(1, len(agg_df) + 1)

            # Create the line plot
            fig = px.line(agg_df, x='match_number', y='is_wicket', color='bowler',
                        labels={'match_number': 'Match Number', 'is_wickets': 'Total Wickets'},
                        title=f'{player} Wickets per Match Against {team}',
                        markers=True)
            
            fig.update_traces(line=dict(color='#FF4B4B'),
                                hovertemplate='<b>Match Number:</b> %{x}<br>' +
                                '<b>Total Wickets:</b> %{y}<extra></extra>',
                  customdata=agg_df[['date']].values)
            fig.update_layout(title={'x': 0.20})
            ...
            with tab2:
                ...
                st.table(bowlers_predict)  # Display the styled DataFrame table in the second tab
                st.markdown(f"""
                    <div style="text-align: center;">
                        <h5>Predictions for {player} against {team}</h5>
                    </div>
